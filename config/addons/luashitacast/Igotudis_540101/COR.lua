...
local sets = {
    ...
    Rolls = { -- it will put on ur DT gear set first then layer this set on for phantom roll/dbl-up, use /dt if you think you need to lock dt set while rolling
        Main = "Lanun Knife",
        Range = "Compensator",
        Head = "Lanun Tricorne +2",
        Neck = "Regal Necklace",
    ...
    Tactician = {Body = "Chasseur's Frac +1"},
    TH = {
		Waist = "Chaac Belt",
...
profile.HandleDefault = function()
    ...
    if gcdisplay.GetToggle("TPgun") then gFunc.EquipSet(sets.Idle_TPgun) end
...
profile.HandleAbility = function()
    ...
    if string.match(ability.Name, "Roll") or string.match(ability.Name, "Double-Up") then
        gFunc.EquipSet(sets.Dt)
        gFunc.EquipSet(sets.Rolls)
        if string.match(ability.Name, "Tactician") then gFunc.EquipSet(sets.Tactician) end
    ...
    elseif string.match(ability.Name, "Shot") and ability.Name ~= "Triple Shot" then
