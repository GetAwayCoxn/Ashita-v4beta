...
function gcinclude.HandleCommands(args)
	if (gcauto ~= nil) then gcauto.HandleCommands(args) end
...
function gcinclude.HandleLevelSync(sets, str)
    if not sets or not str then return end
    local synced = gData.GetBuffCount(143) + gData.GetBuffCount(269)
    if synced == 0 then return end

	local player = AshitaCore:GetMemoryManager():GetPlayer()
	local nearestLevel = math.floor(player:GetMainJobLevel() / 10 ) * 10
	while nearestLevel >= 10 do
		local setToCheck = tostring(str) .. '_' .. tostring(nearestLevel)
        if sets[setToCheck] then
			gFunc.EquipSet(setToCheck)
			return
		else
			nearestLevel = nearestLevel - 10
		end
	end
end
