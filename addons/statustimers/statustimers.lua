...
status_icon_base.update_animation = function(self)
    ...
    if (self.duration.now <= settings.visual_aid.flash_at) then
        ...
        -- in case we're still above the blink duration (or got back there via refresh)
        -- reset the alpha to fully opaque
        local color = bit.band(self.icon.obj:GetBackground():GetColor(), 0x00FFFFFF);
        self.icon.obj:GetBackground():SetColor(bit.bor(0xFF000000, color));

        if (self.bar.obj ~= nil and self:has_visual_aid()) then
            local bar_color = bit.band(self.bar.obj:GetBackground():GetColor(), 0x00FFFFFF);
            self.bar.obj:GetBackground():SetColor(bit.bor(0xFF000000, bar_color));
        end
    end
end
...
local get_status_duration = function(index)
    if (AshitaCore:GetMemoryManager() ~= nil) then
        if (AshitaCore:GetMemoryManager():GetPlayer() ~= nil) then
            ...
            local read_raw_duration = function(i)
                local raw_duration_ptr = AshitaCore:GetPointerManager():Get('player');
                raw_duration_ptr = ashita.memory.read_uint32(raw_duration_ptr);
                local raw_duration_offset = 764 + (index * 4);
                return ashita.memory.read_uint32(raw_duration_ptr + raw_duration_offset);
            end;

            local raw_duration = read_raw_duration(index);
