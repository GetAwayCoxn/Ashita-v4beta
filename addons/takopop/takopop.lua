...
local function cell_count()
	...
	for index = 1, inventory:GetContainerCountMax(0), 1 do
...
ashita.events.register('command', 'command_cb', function (e)
	...
	if (args[2] == 'voidwatch' or args[2] == 'vw') then
		...
		-- inject packet
		AshitaCore:GetPacketManager():AddOutgoingPacket(0x36, menu_item_packet);
...
ashita.events.register('packet_out', 'packet_out_callback1', function (e)
	...
	if (e.id == 0x5B) then
		...
		local event_target_id = struct.unpack('I', e.data, 0x04 + 1);
		local result_id = struct.unpack('I', e.data, 0x08 + 1);
		local target_index = struct.unpack('H', e.data, 0x0C + 1);
		local queue_next_event = struct.unpack('H', e.data, 0x0E + 1);
		local zone_id = struct.unpack('H', e.data, 0x10 + 1);
		local event_id = struct.unpack('H', e.data, 0x12 + 1);

		-- get the entity to use later
		local entity_name = AshitaCore:GetMemoryManager():GetEntity():GetName(target_index);
		...
		if (entity_name == 'Voidwatch Officer') then
			...
			if (result_id == cancel_result) then
				...
				if (colbat_needed > 0 and settings['cells']['colbat']['buy']) then
					...
					local new_packet = (e.data:sub(1, 8) .. string.char(0x02, 0x00, 0x01 + ((colbat_needed * 64) % 256), math.floor((colbat_needed * 64) / 256)) .. e.data:sub(13)):totable();
					AshitaCore:GetPacketManager():AddOutgoingPacket(e.id, new_packet);
				...
				elseif (rubicund_needed > 0 and settings['cells']['rubicund']['buy']) then
					...
					-- option is apart of the third char
					-- count is packed into the third and fourth chars
					local new_packet = (e.data:sub(1, 8) .. string.char(0x02, 0x00, 0x02 + ((rubicund_needed * 64) % 256), math.floor((rubicund_needed * 64) / 256)) .. e.data:sub(13)):totable();
					AshitaCore:GetPacketManager():AddOutgoingPacket(e.id, new_packet);
		...
		elseif (entity_name == 'Planar Rift') then
			...
			if (result_id == cancel_result) then
				...
				if (has_traded or (settings['cell_count'] == 0 and settings['displacer_count'] == 0)) then
					...
					local new_packet = (e.data:sub(1, 8) .. string.char(0x51, 0x00, 0x00, 0x00) .. e.data:sub(13)):totable();
					AshitaCore:GetPacketManager():AddOutgoingPacket(e.id, new_packet);
					has_traded = false;
