    ...
    for n=1, #relics do    
        ...
        for w = 1, #interface.data.ids.weapons.relic do
            ...
            elseif (interface.data.ids.weapons.relic[w][2] == weapon) then
                    ...
                    if stage == 1 then
                        ...
                        if (interface.data.ids.weapons.dynastage2[n][2] == dyna2) then
                            if (interface.data.ids.weapons.dynastage2[n][3] == true) then
                                imgui.TextColored(colors.text1, tostring(interface.data.ids.weapons.dynastage2[n][1]));
                            else
                ...
                elseif stage ~= 7 and stage ~= 9 and stage < 12 then
                    imgui.TableNextColumn();
                    if (weapon == 'Annihilator' and stage == 11) or (weapon == 'Yoichinoyumi' and stage == 11) then
                        -- skips last ID of stupid ranged weapons, thanks SE.
                    elseif (interface.data.ids.weapons.relic[w][3] == true) then
                        imgui.TextColored(colors.text1, tostring(interface.data.ids.weapons.relic[w][1]));
                    else
                    local fwdcount = false;
